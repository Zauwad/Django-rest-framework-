...
#*     def validate_name(self, value):                     #value is the name field's value that the user has provided in their API request. 
#         if len(value) < 2:
...
#*     #These below starts sql execution in database because of serializer.save() in views.py
...
class ReviewSerializer(serializers.ModelSerializer):
    #Because of related_name="reviews" in Review model, Django automatically gives WatchList reverse access to all its reviews via watchlist_instance.reviews.all(). This is also why reviews = ReviewSerializer(many=True, read_only=True) in WatchListSerializer works.
    review_user = serializers.StringRelatedField(read_only=True) #becase here we dont want to show the Review user id, we want to show the username, and to show username we have to query the User model. It will show the return value of User model's __str__ method.

    class Meta:
        model = Review
        # fields = "__all__"
        exclude = ('watchlist',) #This means telling serializer, dont need to ask the user for watchlist_id in json payload. But We will send it from the view later, cz as per model we need watchlist. 
...
#IT HAS CREATE AND UPDATE METHODS BUILT IN, SO YOU DONT NEED TO DEFINE THEM UNLESS YOU WANT TO OVERRIDE THE DEFAULT BEHAVIOR. 
class WatchListSerializer(serializers.ModelSerializer):                    # Serializer class for Movie model, using DRF's ModelSerializer for automatic field generation
    reviews = ReviewSerializer(many=True, read_only = True)  #because of related_name="reviews" in Review model, Django automatically gives WatchList reverse access to all its reviews via watchlist_instance.reviews.all(). This is also why reviews = ReviewSerializer(many=True, read_only=True) in WatchListSerializer works.
...
class StreamPlatformSerializer(serializers.ModelSerializer):
    ...
    watchlist = WatchListSerializer(many=True, read_only=True)   # Without this, fields = "__all__" only outputs a list of IDs (PrimaryKeyRelatedField) via related_name='watchlist'. Explicitly defining WatchListSerializer forces DRF to return full nested movie details instead of just IDs.
