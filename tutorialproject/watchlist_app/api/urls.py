from django.urls import  path, include                    # Imports path function to define individual URL patterns
# from .views import movie_details, movie_list                    # Imports details and list views from local views module
from .views import ReviewCreate, ReviewDetail, ReviewList, WatchListApiView, WatchDetailsApiView, StreamPlatformApiView, StreamPlatformDetailsApiView, StreamPlatformModelViewSet
from rest_framework.routers import DefaultRouter

router = DefaultRouter()
router.register('platforms', StreamPlatformModelViewSet, basename="streamplatform") # router handles both list and details urls auto. no need to write two urls and two classes for this. This is used with ViewSets. 
...
urlpatterns = [                    # List mapping movie URLs to their respective views (API routes)
    ...
    path('', include(router.urls)) #including routers urls
...
# http://127.0.0.1:8000/platforms/3/ ->Here endslash is important
# Django's Standard Best Practice:
# Always add a trailing slash to your custom paths:
# The router automatically forces trailing slashes onto every URL it generates for you behind the scenes.
# Routers: Do not add a slash to the prefix string inside router.register('platforms', ...)—the router handles appending the slashes to the final endpoints automatically.